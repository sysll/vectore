...
for i in range(20):
    ...
    if i>=1:
        ...
        with torch.no_grad():
            ...
            if i >=2:
                # confusion(all_test_target, all_test_output)
                fc_layer = cnn.fc_layer[0]  # nn.Linear 层
                weights_cuda, biases_cuda = fc_layer.weight, fc_layer.bias
                weights = weights_cuda.detach().cpu().numpy().T  # 先转移到CPU，再转换为NumPy数组

                # 绘制前100个神经元的权重
                num_neurons = 300

                # 初始化颜色列表，默认所有权重为蓝色
                colors = ['b'] * num_neurons

                # 设置特定索引位置为红色
                for k in range(0, 30):  # 假设我们只考虑到k=1到9的情况
                    red_indices = [7 + 10 * k, 8 + 10 * k, 9 + 10 * k]
                    for index in red_indices:
                        if index < num_neurons:  # 确保索引不超出范围
                            colors[index] = 'r'

                        # 绘制权重
                plt.figure(figsize=(12, 6))
                plt.bar(range(num_neurons), abs(weights[:num_neurons, 1]), color=colors)
                plt.title('Weights of the first 100 neurons')

...
setup_seed(25)  #10
def get_vector(matrix, n):   #matrix [64, 10, 10]
    sum = []
    all_singular_value = []
    for j in range(matrix.shape[0]):
        ...
        sum.append(part)
        all_singular_value.append(singular_values)

    sum = torch.cat(sum)
    valu = torch.tensor(all_singular_value)
    return sum, valu

def get_bat_v(matrix, n):   #matrix shape[bs, 64, 10, 10]
    sum = torch.zeros((matrix.shape[0], matrix.shape[2] * n * matrix.shape[1]))
    aa = torch.zeros((matrix.shape[0], matrix.shape[1], n))
    for j in range(matrix.shape[0]):
        v, a = get_vector(matrix[j], n)
        sum[j] = v
        aa[j] = a
    return sum, aa
...
class CNNModel(nn.Module):
    def __init__(self, input_size, output_size, k):     #k是特征向量大小
        ...
        self.branch2 = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=18, kernel_size=1),
            nn.Conv1d(in_channels=18, out_channels=36, kernel_size=5*k, stride=5*k),
            nn.Mish(),
        )

        self.fc_layer = nn.Sequential(
            nn.Linear(64*36, 100),  # 更新线性层的输入大小
            nn.Mish(),
            nn.Linear(100, output_size),
        )

    def forward(self, x, a):   #x[100, 1, 3200], a[100, 64, 5]
        x1 = self.branch1(x) #[100, 36, 320]
        MaxFeature = nn.MaxPool1d(kernel_size=5, stride=5)(x1)
        MeanFeature = nn.AvgPool1d(kernel_size=5, stride=5)(x1)
        x2 = MaxFeature+MeanFeature #[100, 36, 64]

        x3 = torch.zeros_like(x)
        # attention = nn.MaxPool1d(kernel_size=5*10, stride=5*10)(x) #[100, 1, 64]
        attention = a.sum(dim=2).unsqueeze(1)
        attention = nn.Softmax(dim=2)(attention)
        # print(attention.shape)
        for i in range(64):
            # print(x[:, :, i*50:i*50+50].shape)
            x3[:, :, i*50:i*50+50] = x[:, :, i*50:i*50+50] * (attention[:,:, i].unsqueeze(2))
        x4 = self.branch2(x3)  # [100, 36, 64]
...
# 定义损失函数和优化器
criterion = nn.MSELoss()
optimizer1 = optim.AdamW(cnn.parameters(), lr=0.001)
...
# #保存两个分数
# import json
# with open('all_predict_score1.json', 'w') as f:
#     json.dump(all_predict_score1, f)
#
# with open('all_predict_score2.json', 'w') as f:
#     json.dump(all_predict_score2, f)
#
# with open('True_label2.json', 'w') as f:
#     json.dump(True_label2, f)